...
RSpec.describe 'Task API' do
  ...
  describe 'GET /tasks' do
    ...
    context "when filter params is sent" do
      let!(:task_1) { create(:task, title: 'The notebook is broken', user_id: user.id ) }
      let!(:task_2) { create(:task, title: 'Buy a new notebook', user_id: user.id ) }
      let!(:task_3) { create(:task, title: 'Fix the door', user_id: user.id ) }
